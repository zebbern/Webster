...
/**
 * Generate chapter URLs for multiple chapters using configured patterns
 */
export function generateMultipleChapterUrls(
  baseUrl: string, 
  startChapter: number, 
  chapterCount: number
): string[] {
  ...
  for (let i = 0; i < chapterCount; i++) {
    const chapterUrl = urlPatternManager.generateChapterUrl(baseUrl, startChapter + i)
    if (chapterUrl) {
